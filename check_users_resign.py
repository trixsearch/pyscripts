...
def fetch_env_from_github():
    """Fetch .env file from GitHub and load it"""
    try:
        print("Fetching configuration from GitHub...")
        response = requests.get(GITHUB_ENV_URL)
        
        if response.status_code == 200:
            # Load the .env content from the response text
            env_content = response.text
            
            # Parse it like a .env file
            ...
            print("✅ Configuration loaded from GitHub")
            return True
        else:
            print(f"❌ Failed to fetch .env from GitHub (Status: {response.status_code})")
            return False
...
if __name__ == "__main__":
    # Fetch .env from GitHub first
    if fetch_env_from_github():
        ...
        if not users_to_check:
            ...
            print(f"Total No. of persons: {len(final_list)} || Till the Time: {currentTime}")
    else:
        print("❌ Failed to load configuration. Script stopped.")



# # old code taking env from local
