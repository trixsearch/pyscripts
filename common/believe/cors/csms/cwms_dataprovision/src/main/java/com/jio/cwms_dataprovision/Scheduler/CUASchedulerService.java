...
public class CUASchedulerService {
	...
	public void cuaAsyncSchedulerService(SchedulerConfigRequest configRequest, String transMode) {
		 LogWrapper.info(getClass(),
	                "CUA Scheduler Triggered | TransMode: " + transMode +
...
//	    }
	    ...
	    switch  (schedulerType == null ? "" : schedulerType.toUpperCase())  {
	        ...
	        case "HOURLY":
	        	...
	        	pageList = acessRequestLogAsyncRepo.findPendingFirstHitsInLastHour("CUA_NEW",
	                    transMode,
	                    conditionCheck,
	                    sites,
	                    useInclude,
...
//	            break;
	    ...
	    if (recordSize == 0) {
	    	LogWrapper.info(getClass(), "No pending CUA_NEW requests found. Scheduler exiting.");
	        return;
	    }

	    LogWrapper.info(getClass(), "Starting CUA_NEW " + transMode + " Batch Processing");
	    ...
	    for (AccessRequestLogEntity req : pageList) {
	        ...
	        try {
	            ...
	            System response = cuaNewService.cuaNewApiCall(req.getRequest(), employeeLog, req.getTransId());
	            String cuaNewRawResponse = employeeLog.getResponse();
	            if(cuaNewRawResponse == null) {
	            	LogWrapper.warn(getClass(), "CUA returned empty response for Emp: " + req.getEmpId());
	            	req.setResponse(cuaNewRawResponse);
	            	req.setResponseTime(LocalDateTime.now());
	            	req.setStatus(CwmsConstants.sysStatusF);
	            	acessRequestLogAsyncRepo.save(req);
	            	return;
	            ...
	            req.setResponse(cuaNewRawResponse);
	            req.setResponseTime(LocalDateTime.now());
	            employeeLog.setResponse(cuaNewRawResponse);
	            ...
	            if ("SUCCESS".equalsIgnoreCase(response.getSystemStatus())) {
	            	LogWrapper.info(getClass(),
                            "CUA API Success | EmpCode: " + req.getEmpId()+" || response : "+ responseJson);
	                finalStatus = CwmsConstants.sysStatusS;
	                
	            }
	            else if ("FAILED".equalsIgnoreCase(response.getSystemStatus())) {
	            	LogWrapper.info(getClass(),
                            "CUA API Failed | EmpCode: " + req.getEmpId()+" || response : "+ responseJson);
	            ...
	            List<System> systemResponse = new ArrayList<>();
	            systemResponse.add(response);
	            resp.setSystem(systemResponse);
	    ...
	    LogWrapper.info(getClass(), "Completed processing  for CUA_NEW batch of " + recordSize + " records." + "| Mode : "+ transMode);
	}
